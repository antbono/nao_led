...
{
  this->client_ptr_ = rclcpp_action::create_client<LedsPlay>(this, "leds_play");
...
void LedsPlayActionClient::headLoop(bool flag)
{
  ...
  if (flag) {
    ...
    // head_goal_handle_ = client_ptr_->async_send_goal(goal_msg,
    // send_goal_options);
  } else {
    // rclcpp_action::ClientGoalHandle<nao_led_interfaces::action::LedsPlay>::SharedPtr
    // handle = head_goal_handle_.get(); auto cancel_result_future =
...
void LedsPlayActionClient::feedbackCallback(
  GoalHandleLedsPlay::SharedPtr, const std::shared_ptr<const LedsPlay::Feedback> feedback)
{
  // TODO
}
